...
_STOP_TOKENS = {"department", "dept", "school", "faculty", "institute", "lab", "laboratory",
                "college", "center", "centre"}
_ADDR_TOKENS = {"p.o.", "po", "box", "street", "st.", "road", "rd.", "avenue", "ave.", "blvd",
                "building", "bldg", "room", "rm", "floor", "fl", "city", "state", "zip", "postal",
                "usa", "us", "united states", "uk", "canada", "japan", "korea", "china", "india",
...
def _clean_aff_text(raw: str) -> str:
    ...
    # 3) 숫자, 우편번호 패턴 제거(너무 공격적이면 완화 가능)
    s = _re.sub(r"\b\d{2,}(-\d{2,})*\b", " ", s)

    # 4) stop/address 토큰 등장 전까지만 남기기
    ...
    # 5) 불필요 토큰 스트립
    tokens = [w for w in s.split() if w.lower() not in _STOP_TOKENS]
    s = " ".join(tokens)
...
async def process(input_csv, output_csv, cache_file, concurrency=20,
    ...
    async def fetch_ror_via_affiliation(session, aff_text: str, sem: asyncio.Semaphore) -> str:
        clean = _clean_aff_text(aff_text or "")
        keys = [("affq", clean), ("aff", aff_text)]
        for k in keys:
            if k in cache:
                return cache[k]
        ...
        # 1) query 우선 (전처리 텍스트)
        if clean:
            qurl = "https://api.ror.org/v2/organizations?" + urlencode({"query": clean})
            rid = await _try(qurl)
            if rid:
                cache[("affq", clean)] = rid
                return rid

        # 2) affiliation 폴백(원문)
        aurl = "https://api.ror.org/v2/organizations?" + urlencode({"affiliation": aff_text})
        rid = await _try(aurl)
        if rid:
            cache[("aff", aff_text)] = rid
            return rid
    ...
    async def _run():
        ...
        async with aiohttp.ClientSession(connector=conn, timeout=timeout) as session:
            # A) OpenAlex I-ID 직통
            if iid_to_entities:
                inst_ids = list(iid_to_entities.keys())
                tasks = [asyncio.create_task(fetch_ror_via_openalex_inst(session, inst_id, sem))
                        for inst_id in inst_ids]
                task_to_inst = dict(zip(tasks, inst_ids))

                async for fut in atqdm(asyncio.as_completed(tasks), total=len(tasks),
                                    desc="Fetching ROR by OpenAlex I-ID", unit="task"):
                    inst_id = task_to_inst.get(fut)
                    ror_id = await fut
                    if not inst_id or not ror_id:
                        continue
            ...
            if name_to_entities:
                ...
                async for fut in atqdm(asyncio.as_completed(tasks), total=len(tasks),
                    ...
                    for etype, idx, auth, obj in name_to_entities[name]:
                        if etype == 'aff':
                            if ror_id and not obj.get('ror'):
                                obj['ror'] = _to_ror_url(ror_id)
                                augment_count += 1
                            ...
                            for aff in (auth.get('affiliations') or []):
                                if aff.get('raw_affiliation_string') == obj:
                                    urlval = _to_ror_url(ror_id)
                                    if not aff.get('ror'):
                                        aff['ror'] = urlval
                                        augment_count += 1
                                    elif aff.get('ror') and not str(aff['ror']).startswith("http") and urlval:
                                        aff['ror'] = urlval
                                    updated = True
                                    break
                            ...
                            if not updated:
                                ...
                                # 중복 방지: 모두 URL 형태로 정규화하여 비교
                                existing_rors = {
                                    _to_ror_url(inst.get('ror') or "").strip().lower()
                                    for inst in (auth['institutions'] or [])
                                }
                                urlval = _to_ror_url(ror_id)

                                if urlval and urlval.strip().lower() not in existing_rors:
                                    auth['institutions'].append({'ror': urlval})
                                    augment_count += 1
                                    updated = True

                            if not updated and ror_id:
                                # (C) 최후 폴백: 문자열 필드에라도 URL로 남겨 정규식/후속단계가 잡도록
                                prev = (auth.get('raw_affiliation_ror') or "").strip().lower()
                                urlval = _to_ror_url(ror_id)
                                if urlval and urlval.lower() != prev:
                                    auth['raw_affiliation_ror'] = urlval
                                    augment_count += 1
