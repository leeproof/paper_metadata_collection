...
- 기본값을 속도 우선으로 조정(physics=False, layout='fr', use_quantile=True, q=0.85)
"""
...
def to_pyvis(G: nx.Graph, html_path: str,
             ...
             base_size: float = 6.0,
             physics: bool = False,
             positions: Optional[Dict[Any, Tuple[float, float]]] = None) -> str:
    """
    G를 Pyvis Network로 변환 후 html 저장.
    positions 가 주어지면 물리 시뮬 없이 고정 좌표 사용.
    """
    net = Network(height="800px", width="100%", notebook=False,
                  bgcolor="#111111", font_color="#eaeaea", directed=False)
    ...
    for n in G.nodes():
        w = strengths.get(n, 1)
        size = base_size + scale * (w ** 0.5)
        label = n if len(str(n)) <= 24 else (str(n)[:21] + "…")  # 과한 라벨 길이 방지
...
def main(input_csv: str,
         ...
         top_k: Optional[int] = 400,
         layout: str = "fr",
         physics: bool = False) -> Optional[str]:
    """
    ...
    return to_pyvis(Gt, html_path, scale=4.0, base_size=6.0, physics=physics, positions=pos)
