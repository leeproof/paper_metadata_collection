...
def build_graph_from_csv(input_csv, org_col="org_names"):
    df = pd.read_csv(
        input_csv,
        usecols=[org_col],
        dtype={org_col: str},
    ...
    if org_col not in df.columns:
        raise ValueError(f"'{org_col}' 컬럼이 없습니다.")
    df["inst_list"] = df[org_col].apply(parse_and_clean)

    node_w, edge_w = defaultdict(int), defaultdict(int)
...
def to_pyvis(G, html_path, scale=4, base_size=6, physics=False):
    ...
    # 성능 최적화 옵션: 라벨 최소화, 드래그/줌 시 엣지 숨김, 물리 끄기(기본)
    net.set_options("""
    const options = {
      nodes: {
        shape: "dot",
