...
def trim_graph(G: nx.Graph,
    ...
    print("[weights]", Counter(weights).most_common(10))
    if min_edge is None:
        if use_quantile:
    ...
    # LCC + 2nd CC까지(아주 약간만 늘리기)
    if lcc_only and H.number_of_nodes() > 0:
        try:
            comps = list(nx.connected_components(H))
            if comps:
                comps_sorted = sorted(comps, key=len, reverse=True)
                keep = set().union(*comps_sorted[:2])   # ← 여기 숫자 2가 “아주 약간” 증가분
    ...
    if top_k and H.number_of_nodes() > top_k:
        strengths = {n: sum(d.get("weight", 1) for _, _, d
                            in H.edges(n, data=True)) for n in H.nodes()}
