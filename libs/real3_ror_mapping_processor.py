...
async def process(input_csv, output_csv, cache_file, concurrency=20,
    ...
    async def _fetch_all():
        ...
        async with aiohttp.ClientSession(connector=conn, timeout=timeout) as session:
            ...
            tasks = [
                asyncio.create_task(fetch_ror_for_query(session, q, sem))
                for q in queries
            ]
            task_to_query = dict(zip(tasks, queries))

            async for fut in atqdm(
                asyncio.as_completed(tasks),
                total=len(tasks),
                desc="Fetching ROR by affiliation",
                unit="task",
            ):
                q = task_to_query.get(fut)
                try:
                    rid = await fut
                except Exception as e:
                    print(f"❌ query={q!r} fetch 실패: {e}")
                    rid = ""
                if q and rid:
    ...
    for idx in rows_needing_ror:
        ...
        # 1) affiliations[*].ror 에 넣기
        for auth in auth_list:
            for aff in (auth.get("affiliations") or []):
                cur = (aff.get("ror") or "").strip()
                if not cur or not cur.startswith("http"):
                    aff["ror"] = u
                    injected = True
                    augment_count += 1
                    _log_ror(
                        "affiliation",
                        _safe_rowid(df, idx),
                        f"raw='{str(aff.get('raw_affiliation_string',''))[:120]}'",
                        u,
                    )
                    break
            if injected:
        ...
        # 2) affiliations에 못 넣었으면 institutions[*].ror 로라도 넣기
        if not injected:
            first = auth_list[0]
            inst_list = first.setdefault("institutions", [])
            existing = {
                _to_ror_url((inst.get("ror") or "")).strip().lower()
                for inst in inst_list
            }
            if u.strip().lower() not in existing:
                inst_list.append({"ror": u})
                injected = True
                augment_count += 1
                _log_ror(
                    "institutions-fallback",
                    _safe_rowid(df, idx),
                    f"author={first.get('author', {}).get('display_name','')}",
                    u,
                )

        # 3) raw_affiliation_ror 필드에도 동기화(있으면 덮어씀)
        if injected:
            for auth in auth_list:
                prev = (auth.get("raw_affiliation_ror") or "").strip().lower()
                if prev != u.lower():
                    auth["raw_affiliation_ror"] = u

        # 수정된 authorships를 df에 반영
        if injected:
