...
def main(issns, year_start, year_end, email, prefix='', 
    ...
    enriched_from_empty = sum(1 for w in unique_works if w['id'] in empty and w.get('doi'))
    ...
    # ✅ 메트릭 JSON 저장 (추가)
    # ================================
    # app.py 쪽에서 집계해 쓰기 쉽도록, 실행 구간(ISSN 묶음, 연도범위) 단위로 메트릭을 남깁니다.
    # 파일명: {prefix}_{year_start}_{year_end}_metrics.json
    try:
        import json, os
        ...
        if os.path.exists(metrics_path):
            try:
                with open(metrics_path, "r", encoding="utf-8") as mf:
                    metrics = json.load(mf) or {}
            except Exception:
                metrics = {}

        # 필요한 지표만 심플하게 저장
        metrics.update({
            "total_collected": int(len(unique_works)),             # 최종 합본에 남은 문헌 수
            "doi_missing_initial": int(len(empty)),                # 최초 DOI 결측 수
            "doi_enriched_from_empty": int(enriched_from_empty),   # 결측 -> 보강 성공 수
            "doi_enrich_rate": (
                (enriched_from_empty / len(empty)) if len(empty) > 0 else None
            )
        })
