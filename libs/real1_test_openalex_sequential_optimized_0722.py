...
def main(issns, year_start, year_end, email, prefix='', 
    ...
    missing_doi_ids = {w['id'] for w in unique_works if not (w.get('doi') or '').strip()}
    sidecar_path = Path(f"{prefix}_{year_start}_{year_end}_missing_doi_ids.json")
    with open(sidecar_path, "w", encoding="utf-8") as f:
        json.dump(sorted(missing_doi_ids), f, ensure_ascii=False)
    ...
    _update_metrics(
        ...
        doi_missing_before_editorial=missing_doi_count,
