...
def main(input_csv, output_csv, prefix, year_start, year_end,
    ...
    import re as _re
    _rx = _re.compile(r'https?://ror\.org/[0-9a-z]+', _re.I)

    # before: authorships 텍스트에 ror URL이 하나도 없는 행 수
    ror_missing_before = int(
        (df['authorships'].astype(str).apply(lambda s: len(_rx.findall(s)) == 0)).sum()
    )

    # after: 추출된 ror 리스트가 비어 있는 행 수 (real4가 만든 df['ror'] 기준)
    ...
    print(f"[rea4] WROTE rows: {len(df)} to {output_csv}")
