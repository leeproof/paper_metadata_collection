...
from libs import real1_test_openalex_sequential_optimized_0722_non_imputation as real1
...
def run_pipeline(issns: List[str], year_start: int, year_end: int,
    ...
    # 3. real4: ROR 추출 (real2 결과에서 바로 추출)
    print("[3/6] ROR 추출 및 통계...")
    real4.main(input_csv=csv_file, output_csv=csv_ror_extract, prefix=prefix, year_start=year_start, year_end=year_end)

    # 4. real5: ROR ID → 기관명 매핑
    print("[4/6] ROR ID → 기관명 매핑...")
    ...
    # 5. real6: 협력 네트워크 시각화 (HTML 생성)
    html_path = None
    if make_html:
        print("[5/6] 협력 네트워크 시각화...")
