...
def _summary_metrics_common(final_csv_path: str):
    ...
    def sum_metrics(keys):
        ...
        # prefix_all: 하이픈으로 합쳐진 '통합 prefix'
        prefix_all = Path(base).name[:-(len(re.search(r"_(\d{4})_(\d{4})$", base).group(0)))] if re.search(r"_(\d{4})_(\d{4})$", base) else ""
        parts = prefix_all.split("-") if prefix_all else []

        for y in range(y0, y1+1):
            # 1) 통합 prefix 파일 먼저 시도 (실제 real4가 이렇게 저장)
            cand = [Path(f"{prefix_all}_{y}_{y}_metrics.json")] if prefix_all else []
            # 2) (존재하면) 개별 prefix 파일도 합산
            cand += [Path(f"{p}_{y}_{y}_metrics.json") for p in parts]
            for mf in cand:
                if mf.exists():
                    try:
    ...
    if json_merged.exists():
        ...
        if found:
            total_collected = acc["json_rows", 0]
            removed_authorships_empty = acc["authorships_removed_empty_list", 0]
