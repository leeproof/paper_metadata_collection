...
def _summary_metrics_common(final_csv_path: str):
    ...
    def sum_metrics(keys, search_dirs, allowed_names=None):
        ...
        # 중복 차단: (경로) + (저널, y1, y2) + (복사본 접미사 제거한 파일명)
        seen_paths = set()
        seen_names = set()   # 파일명 단위 중복 차단
        seen_triplets = set()  # (key, y1, y2) 단위 중복 차단

        # 예: "journal_2021_2021_metrics.csv" 또는 "journal_2021_2021_metrics (1).csv"
        rx_triplet = re.compile(r'^(?P<key>.+?)_(?P<y1>\d{4})_(?P<y2>\d{4})_metrics(?: \(\d+\))?\.csv$', re.IGNORECASE)
        ...
        for base in search_dirs:
            ...
            for mf in base.rglob("*_metrics.csv"):
                # 1) 이번 실행의 대상 파일만 허용 (복사본 접미사 제거하여 비교)
                base_name = _strip_copy_suffix(mf.name)
                if allowed_names and (base_name not in allowed_names):
                    continue

                # 2) (저널, y1, y2) 단위 중복 차단
                m = rx_triplet.match(mf.name)
                if m:
                    triplet = (m.group("key").lower(), int(m.group("y1")), int(m.group("y2")))
                    if triplet in seen_triplets:
                        continue
                    seen_triplets.add(triplet)

                # 3) 파일명 중복 차단 (경로가 달라도 같은 이름이면 1회만)
...
def _summary_top_nodes_and_edges(final_csv_path: str):
    ...
    """
    import pandas as pd, ast, itertools
