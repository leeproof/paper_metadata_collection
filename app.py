...
if "view" not in st.session_state:
    _default = st.query_params.get("view", "미리보기")
    st.session_state.view = _default if _default in ("미리보기", "다운로드", "Summary") else "미리보기"
...
def _summary_metrics_common(final_csv_path: str):
    ...
    def sum_metrics(keys):
        ...
        # csv와 같은 폴더 및 workdir_tmp에서 모든 _metrics.json 수집
        root_dir = Path(root_base).parent
        search_dirs = [root_dir, root_dir / "workdir_tmp"]
        candidates = []
        for d in search_dirs:
            if d.exists():
                candidates.extend(d.glob("*_metrics.json"))
...
def app():
    ...
    def _render_preview_block(sel_csv: str):
        ...
        # ▼ 네트워크 미리보기(임베드) — degree / eigenvector
        with st.expander("네트워크 미리보기 (Degree / Eigenvector)", expanded=False):
    ...
    if run_btn:
        ...
        # ✅ 라디오 박스 복원: 미리보기 / 다운로드 / Summary
        view = st.radio(
            "보기 선택",
            ["미리보기", "다운로드", "Summary"],
            index=["미리보기","다운로드","Summary"].index(st.session_state.view),
            key="_view_radio",
            horizontal=True,
