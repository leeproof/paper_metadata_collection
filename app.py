...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    """
    Summary를 계산할 때 aggregated 파일 대신(또는 그게 없을 때) 모든 *_metrics.csv를 바로 합산하여 summary값을 제공.
    - sel_csv_list: final CSV 경로 리스트 (스트림릿에서 전달되는 값)
    - 반환: 한 행짜리 DataFrame (UI가 기대하는 컬럼 이름으로)
    """
    import pandas as pd
    from pathlib import Path
    import csv, sys, re
    ...
    # 안전: 입력이 없으면 0 채운 빈 리포트 반환
    if not sel_csv_list:
        return pd.DataFrame([{k: (0 if "수" in k or "행" in k else None) for k in out_keys}])

    # 1) 검색할 기본 디렉터리 집합 구성 (parent, parent/workdir_tmp, 상위 ancestor 들)
    scan_dirs = set()
    ...
    # 디버그 로그(터미널에 표시됨)
    print(f"[summary_multi] found {len(metrics_files)} *_metrics.csv files")
    for mf in metrics_files:
        print("  -", mf)

    # 3) metrics 파일들에서 합산 (키 변형 관대히 처리)
    agg = {}
    for mf in metrics_files:
        try:
            ...
            try:
                ...
                print(f"[summary_multi] failed reading {mf}: {e}", file=sys.stderr)
                continue

        # DataFrame layout 처리: ('key','value') or 2-column
        ...
        for k, v in rows:
            ...
            if kn in ("total", "total_collected", "json_rows"):
                ...
                key = "doi_missing"
            elif kn in ("doi_enriched", "doi_enrich", "doi_added"):
                key = "doi_enriched"
            elif kn in ("ror_missing", "ror_missing_count"):
                key = "ror_missing"
            elif kn in ("ror_enriched", "ror_added"):
    ...
    # 4) 만약 metrics 파일이 전혀 없으면, final CSV를 직접 읽어 계산(폴백)
    if not agg:
        # final CSV에서 직접 계산: 최적의 컬럼명을 유추해서 집계
        total_collected = 0
        editorial_removed = 0
        final_rows = 0
        doi_missing = 0
        ...
        for final_csv in sel_csv_list:
            try:
                df = pd.read_csv(final_csv, dtype=str, encoding='utf-8-sig', on_bad_lines='warn')
                total_collected += df.shape[0]
                final_rows += df.shape[0]
                cols = {c.lower(): c for c in df.columns}
                # authorship 비어있음 카운트
                auth_col = None
                for c in cols:
                    if "authorship" in c or "author" in c:
                        auth_col = cols[c]; break
                if auth_col:
                    # 빈 또는 []로 표기된 항목을 편의적으로 카운트
                    editorial_removed += int((df[auth_col].fillna("").astype(str).str.strip().isin(["", "[]"])).sum())
                # DOI 결측 추정
                doi_col = None
                for c in cols:
                    if "doi" == c or "doi" in c:
                        doi_col = cols[c]; break
                if doi_col:
                    doi_missing += int(df[doi_col].isna().sum() + (df[doi_col].astype(str).str.strip()=="" ).sum())
                # ROR 결측 추정 (org/ror 컬럼 탐색)
                ror_col = None
                for c in cols:
                    if "ror" in c:
                        ror_col = cols[c]; break
                if ror_col:
                    ror_missing += int(df[ror_col].isna().sum() + (df[ror_col].astype(str).str.strip()=="" ).sum())
            except Exception:
                continue
        agg = {
            "total_collected": total_collected,
            "authorships_removed_empty_list": editorial_removed,
    ...
    try:
        ...
        for p in sel_csv_list:
            try:
                ...
                pass
            try:
                df_final = pd.read_csv(p, dtype=str, encoding='utf-8-sig', on_bad_lines='warn')
