...
def _summary_metrics_common(final_csv_path: str):
    ...
    # export 파일로 '전체 수집 논문 수' 추정하지 않도록 설정
    is_export = final_csv.name.endswith("_export.csv")
    ...
    def sum_metrics(keys):
        ...
        # ✅ 0) 기간 통합 metrics 먼저 시도 — 성공했다면 **바로 반환**(연도별 합산 건너뜀)
        if prefix_all:
            merged = Path(f"{prefix_all}_{y0}_{y1}_metrics.json")
            if merged.exists():
                try:
                    ...
                    return acc, True   # ★ 여기서 즉시 반환 (중복합산 방지)
                except Exception:
                    pass

        # 1) 연도별 합산 — ✅ 후보 중복 제거 (prefixes에서 prefix_all과 같은 값 제외)
        for y in range(y0, y1 + 1):
            candidates = []
            if prefix_all:
                candidates.append(Path(f"{prefix_all}_{y}_{y}_metrics.json"))
            for px in prefixes:
                if px and px != prefix_all:  # ★ 같은 값이면 추가하지 않음
                    candidates.append(Path(f"{px}_{y}_{y}_metrics.json"))
    ...
    # 1) 전체 수집 수 & authorships=[] 삭제 수  ——  ⭐ metrics 최우선, export면 CSV fallback 금지
    total_collected = None
    removed_authorships_empty = None

    # (A) metrics에서 우선 수집 (real1/real2가 기록)
    acc, found = sum_metrics([
        "total_collected",                   # real1: 중복 제거 후 개수 (로그의 '수집된 논문 수 (중복 제거 후)')
        "final_csv_rows",                    # real2: CSV 최종 행 수 (참고값)
        "authorships_removed_empty_list",    # real2: authorships '[]' 필터 삭제 수
    ...
    # (C) 최후 fallback: CSV 길이 —— ❗단, export 요약본일 경우에는 금지
    ...
    df_out = pd.DataFrame([{
        ...
        "ror 보강률": (None if (ror_enrich_rate is None and _ror_enrich_rate_fallback is None)
                    else round((ror_enrich_rate*100 if ror_enrich_rate is not None else _ror_enrich_rate_fallback), 2)),
...
def app():
    ...
    if run_btn:
        ...
        with st.expander("Summary", expanded=True):
            ...
            summary_df = _summary_metrics_common(sel["csv"])
            st.dataframe(summary_df, width="stretch")

            # ✅ 인덱스 없는 CSV 다운로드 (Unnamed: 0 방지)
            csv_bytes = summary_df.to_csv(index=False).encode("utf-8-sig")
            st.download_button(
                "Download Summary CSV",
                data=csv_bytes,
                file_name=Path(sel["csv"]).stem + "_summary.csv",
                mime="text/csv",
            )
