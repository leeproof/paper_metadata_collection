...
def app():
    ...
    def _render_summary_block(sel_csv_or_list):
        ...
        def _single_top10(sel_csv_path: str):
            """
            Top10은 최종 CSV의 실제 컬럼만 사용:
            - 노드: org_names (리스트)
            - 엣지: ror (리스트) → 각 논문 내 무방향 ROR 페어 빈도
            """
            # 기존 헬퍼가 있으면 그대로 사용(전체 로직 유지)
            try:
            ...
            # ---------------- Nodes: org_names (list-like) ----------------
            nodes_df = None
            if "org_names" in df.columns:
                def _listify_names(x):
                    if x is None:
                        return []
                ...
                series = df["org_names"].apply(_listify_names)
                if not series.empty:
                    expl = series.explode()
                    expl = expl[expl.notna() & (expl.astype(str).str.strip() != "")]
                    if not expl.empty:
                        nodes_df = (
                            expl.astype(str).str.strip()
                                .value_counts().head(10).rename_axis("node")
                                .reset_index(name="count")
                        )

            # ---------------- Edges: ror (list-like → undirected pairs) ----------------
            edges_df = None
            if "ror" in df.columns:
                def _listify_ror(x):
                    ...
                    if isinstance(x, list):
                        ...
                        if not s:
                            ...
                            try:
                                v = ast.literal_eval(s)
                                arr = v if isinstance(v, list) else [s]
                            except Exception:
                                arr = [s]
                    # URL → ROR ID 정규화, 소문자
                    out = []
                ...
                cnt = Counter()
                for lst in df["ror"].apply(_listify_ror):
                    if len(lst) < 2:
                        continue
                    for a, b in itertools.combinations(lst, 2):
                        key = (a, b) if a <= b else (b, a)  # 무방향 정규화
                        cnt[key] += 1

                if cnt:
                    edges_df = pd.DataFrame(
                        [{"source_ror": a, "target_ror": b, "weight": w}
                        for (a, b), w in cnt.most_common(10)]
                    )
