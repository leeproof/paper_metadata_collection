...
# === quick debug (임시; 테스트 후 삭제) ===
with st.sidebar.expander("🔧 Debug · Secrets"):
    st.write("Has GDRIVE_FOLDER_ID:", "GDRIVE_FOLDER_ID" in st.secrets)
    st.write("Has gdrive_service_account:", "gdrive_service_account" in st.secrets)
    if "gdrive_service_account" in st.secrets:
        s = st.secrets["gdrive_service_account"]
        st.write("Type:", type(s).__name__)
        st.write("Length:", len(str(s)))
# === /quick debug ===
