...
def _summary_top_nodes_and_edges(final_csv_path: str):
    ...
    # candidates가 2개/1개인 경우도 열 정렬 보정 (그대로 유지)
    order = []
    ...
    df_edges = df_edges[[c for c in [f"{m}: 엣지" for m in ["degree","eigenvector","betweenness", "closeness"]] if c in df_edges.columns] + (["동시출현 수"] if "동시출현 수" in df_edges.columns else [])]
