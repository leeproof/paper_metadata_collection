...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    """
    Robust aggregator for multiple final CSVs:
    - Search strategy: for each final_csv, scan several ancestor dirs and common workdir locations,
      and recursively collect all '*_metrics.csv' found.
    - Accepts multiple metric key name variants and aggregates numeric keys.
    - Recomputes rates (DOI/ROR) from aggregated counts.
    - Returns a one-row DataFrame matching the UI expected columns.
    """
    import pandas as pd
    from pathlib import Path
    import csv, sys, re
    ...
    # Collect a set of directories to search.
    scan_dirs = set()
    for p in sel_csv_list:
        try:
            ...
            continue
        # add the immediate parent and common subfolder names
        scan_dirs.add(pth.parent)
        scan_dirs.add(pth.parent / "workdir_tmp")
        # also add ancestor levels up to 4 (to catch journal/year/subfolders)
        ancestor = pth.parent
        for _ in range(4):
            if ancestor is None or ancestor == ancestor.parent:
                break
            ancestor = ancestor.parent
            scan_dirs.add(ancestor)
            scan_dirs.add(ancestor / "workdir_tmp")
    # Also try some environment / common roots if available
    try:
        import os
        env_roots = []
        # common env var that may point to workspace
        for k in ("LOCAL_WORKDIR", "WORKDIR", "WORK_DIR", "DATA_DIR"):
            v = os.environ.get(k)
            if v:
                env_roots.append(Path(v))
        # optionally from streamlit secrets if available
        try:
            import streamlit as _st
            sroot = None
            if hasattr(_st, "secrets") and isinstance(_st.secrets, dict):
                sroot = _st.secrets.get("LOCAL_WORKDIR") or _st.secrets.get("WORKDIR")
            if sroot:
                env_roots.append(Path(sroot))
        except Exception:
            pass
        for er in env_roots:
            if er:
                scan_dirs.add(Path(er))
                scan_dirs.add(Path(er) / "workdir_tmp")
    except Exception:
        pass
    ...
    # find all *_metrics.csv recursively under each scan dir (dedupe)
    metrics_files = []
    ...
    # Debug output: helpful to check in logs which files were found
    print(f"[summary_multi] found {len(metrics_files)} metrics files")
    for mf in metrics_files:
        print("  -", mf)

    # Aggregate numeric fields from metrics files, tolerant to various key names
    agg = {}
    for mf in metrics_files:
        try:
            df = pd.read_csv(mf, encoding='utf-8-sig', on_bad_lines='warn')
        except Exception as e:
            # fallback read with csv module
            try:
                rows = []
                with open(mf, 'r', encoding='utf-8-sig', newline='') as fh:
                    rdr = csv.reader(fh)
                    hdr = next(rdr, None)
                    for r in rdr:
                        if len(r) >= 2:
        ...
        # support two layouts: 'key'/'value' columns, or simple two-col CSV
        ...
        for k, v in rows:
            ...
            if kn == "":
                continue
            kn_low = kn.lower().strip()
            num = to_float_safe(v)
            if num is None:
                continue
            # canonicalize
            if kn_low in ("total", "total_collected", "json_rows"):
                key = "total_collected"
            elif kn_low in ("final_csv_rows", "final_rows"):
                key = "final_csv_rows"
            elif kn_low in ("authorships_removed_empty_list", "authorships_removed", "editorial_removed"):
                key = "authorships_removed_empty_list"
            elif kn_low in ("doi_missing", "doi_missing_count"):
                key = "doi_missing"
            elif kn_low in ("doi_enriched", "doi_enrich", "doi_added"):
                key = "doi_enriched"
            elif kn_low in ("ror_missing", "ror_missing_count"):
                key = "ror_missing"
            elif kn_low in ("ror_enriched", "ror_added"):
                key = "ror_enriched"
            else:
                key = kn_low
            agg[key] = agg.get(key, 0.0) + num

    # Try to compute ror_missing_before by scanning the final CSVs' authorships (best-effort)
    ror_missing_before_total = None
    try:
        rx = re.compile(r'https?://ror\\.org/[0-9a-z]+', re.I)
        total_before = 0
        for final_csv in sel_csv_list:
            try:
                tdf = pd.read_csv(final_csv, dtype={"authorships": str}, encoding='utf-8-sig', on_bad_lines='warn')
                if "authorships" in tdf.columns:
                    before = tdf["authorships"].astype(str).apply(lambda s: rx.findall(s) if s and s.strip() else [])
                    total_before += int((before.str.len() == 0).sum())
            except Exception:
                pass
        ror_missing_before_total = total_before
    except Exception:
        ror_missing_before_total = None

    # Compute final aggregated fields and recalc rates
    ...
    if (not ror_enriched) and (ror_missing_before_total is not None) and (ror_missing is not None):
        try:
            ror_enriched_calc = max(0, (ror_missing_before_total or 0) - (ror_missing or 0))
            ror_enriched = int(ror_enriched_calc)
        except Exception:
            pass
    if denom_r:
    ...
    out = {
        ...
        "DOI 보강률": doi_rate,
        "ROR ID 결측 수(합산)": (ror_missing_before_total if ror_missing_before_total is not None else ror_missing),
    ...
    # best-effort node/edge calculation
    try:
        nodes = set(); edges = set()
        for p in sel_csv_list:
            try:
                cent = _find_centrality_csvs(p)
                if cent and Path(cent).exists():
                    cdf = pd.read_csv(cent)
                    node_col = "node" if "node" in cdf.columns else cdf.columns[0]
                    nodes.update(cdf[node_col].astype(str).tolist())
            except Exception:
                pass
            try:
                df_final = pd.read_csv(p, encoding='utf-8-sig', on_bad_lines='warn')
                if "org_names" in df_final.columns:
                    for s in df_final["org_names"].astype(str).dropna():
