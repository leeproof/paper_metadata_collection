...
# === In-Memory Run Registry (세션에만 보관; 디스크 재참조/누적 없음) ===
import time

def _mem_runs():
    import streamlit as st
    if "_mem_runs" not in st.session_state:
        st.session_state._mem_runs = []  # [{label, final_df, summary_df, downloads:{name->bytes}, ts}]
    return st.session_state._mem_runs

def register_run_in_memory(label: str, final_df, summary_df, downloads: dict):
    _mem_runs().insert(0, {
        "label": label,
        "final_df": final_df,
        "summary_df": summary_df,
        "downloads": downloads,  # {"final.csv": bytes, "summary.csv": bytes, ...}
        "ts": time.time(),
    })

def list_runs_in_memory(limit: int = 50):
    return _mem_runs()[:limit]

# === 실행 전 정리: 캐시/임시 파일/누적 상태만 비움 (_mem_runs는 유지) ===
def _clear_previous_outputs_and_session():
    import streamlit as st
    from pathlib import Path
    ...
    # 누적 합산의 원인이 되는 임시 상태 제거 (메모리 레지스트리는 건드리지 않음)
    for k in ["runs", "selected_runs", "last_summary_df", "_reran_once"]:
        if k in st.session_state:
            st.session_state.pop(k, None)

    # 디스크 임시 산출물 정리(원본 storage는 건드리지 않음)
    try:
        workdir = Path.cwd() / "workdir_tmp"
        if workdir.exists():
            for pat in ["*_summary*.csv", "*_metrics*.csv", "*_ror_extract_name*.csv"]:
...
# === 메모리 우선 렌더: 있으면 메모리, 없으면 기존 경로 fallback ===
def _render_preview_from_memory_or_path(sel):
    import streamlit as st, pandas as pd
    try:
        # sel이 dict이면 메모리 우선
        if isinstance(sel, dict) and sel.get("final_df") is not None:
            df = sel["final_df"]
            total_rows = len(df)
            st.caption(
        ...
        pass
    # fallback: 기존 경로 기반
    _render_preview_block(sel["csv"] if isinstance(sel, dict) else sel)

def _render_downloads_from_memory_or_path(sel):
    import streamlit as st
    # 메모리 다운로드 우선
    if isinstance(sel, dict) and sel.get("downloads"):
        st.subheader("Downloads")
        for fname, blob in sel["downloads"].items():
            st.download_button(
                label=f"다운로드: {fname}",
                data=blob,
                file_name=fname,
        ...
        return
    # fallback: 기존 경로 기반
    _render_download_block(sel["csv"] if isinstance(sel, dict) else sel, sel.get("html") if isinstance(sel, dict) else "")

def _render_summary_from_memory_or_path(sel):
    import streamlit as st, pandas as pd
    # 메모리 요약 우선
    if isinstance(sel, dict) and sel.get("summary_df") is not None:
        st.subheader("Summary")
        st.dataframe(sel["summary_df"], use_container_width=True)
        return
    # fallback: 기존 경로 기반(단일 실행 요약)
    _render_summary_block(sel["csv"] if isinstance(sel, dict) else sel)
...
def _summary_top_nodes_and_edges(final_csv_path: str):
    ...
    order = []
    for c in ["degree","eigenvector","betweenness"]:
        if f"{c}: 노드" in df_nodes.columns: order.append(f"{c}: 노드")
        if f"{c}: 엣지" in df_edges.columns: order.append(f"{c}: 엣지")
    df_nodes = df_nodes[[c for c in [f"{m}: 노드" for m in ["degree","eigenvector","betweenness"]] if c in df_nodes.columns]]
    df_edges = df_edges[[c for c in [f"{m}: 엣지" for m in ["degree","eigenvector","betweenness"]] if c in df_edges.columns]]
...
def app():
    ...
    def _render_summary_unified(sel):
        # 누적 합산 금지: 항상 '선택된 1개 실행'만 요약
        _render_summary_from_memory_or_path(sel)  # 메모리 우선, 없으면 기존 단일 요약
    ...
    if run_btn:
        # 🔒 실행 직전: 과거 산출물/세션 상태 정리(메모리 실행목록은 유지)
        _clear_previous_outputs_and_session()
    ...
    if st.session_state._job_state == "running" and st.session_state._job:
        ...
        try:
            # Summary용 DF (new.py의 요약 생성기 사용: 경로→DF)
            from new import build_summary_from_metrics_for_final
            summary_df = build_summary_from_metrics_for_final(final_csv)
            if isinstance(summary_df, str):  # 방어: 경로가 올 수 있으면 DF로 읽기
                summary_df = pd.read_csv(summary_df)
        except Exception:
            summary_df = None

        # 다운로드 바이너리(디스크에 쓰지 않고 메모리 직렬화)
        downloads = {}
        try:
            if final_df is not None:
                downloads["final.csv"] = final_df.to_csv(index=False).encode("utf-8-sig")
            if summary_df is not None:
                downloads["summary.csv"] = summary_df.to_csv(index=False).encode("utf-8-sig")
        except Exception:
            pass

        # 기존 runs에도 기록(기존 UI 유지), 단 메모리 결과 필드 추가
        st.session_state.runs.append({
            "label": label, "csv": final_csv, "html": final_html,
            "final_df": final_df, "summary_df": summary_df, "downloads": downloads,
            "start": start_time.strftime('%Y-%m-%d %H:%M:%S'),
            "end": end_time.strftime('%Y-%m-%d %H:%M:%S'),
            "duration": str(duration)
        })

        # 세션 메모리 레지스트리에도 등록(사이드바 ‘실행 결과 선택(세션 메모리)’용)
        try:
            register_run_in_memory(label, final_df, summary_df, downloads)
        except Exception:
            pass
        ...
        if st.session_state.runs:
            ...
            if view == "Preview":
                _render_preview_from_memory_or_path(sel)
            if view == "다운로드":
                _render_downloads_from_memory_or_path(sel)
            if view == "Summary":
                _render_summary_from_memory_or_path(sel)

            # 최근 선택 CSV를 세션에 보관(선택 유지용)
            st.session_state["last_csv"] = sel.get("csv", None)
        else:
