...
def _summary_metrics_common(final_csv_path: str):
    ...
    # ---- 10) 출력 테이블 구성 + 포맷
    first_rows = (0 if total_collected is None else total_collected)                 # 최초 = total_collected
    final_rows = (len(df_final) if 'df_final' in locals() else 0)                    # 최종 = 최종 CSV 행수

    df_out = pd.DataFrame([{
        "최초 수집 논문 수": first_rows,
        "최종 수집 논문 수": final_rows,
        "Editorial Material 삭제 수": editorial_total,
        "최종 CSV 행 수(합계)": final_rows,
        "DOI 결측 수(합산)": doi_missing_show,
        ...
        "ROR ID 결측 수(합산)": (ror_missing_sum if ror_missing_sum is not None else ror_missing_before),
    ...
    for col in ["최초 수집 논문 수","최종 수집 논문 수","Editorial Material 삭제 수","DOI 결측 수(합산)","DOI 보강 수(합산)",
                "ROR ID 결측 수(합산)","ROR ID 보강 수(합산)","노드 수","엣지 수"]:
        if col in df_out.columns:
