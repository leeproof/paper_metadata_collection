...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    ...
    각 실행의 루트 폴더(및 하위 폴더)를 모두 검색하여 모든 "*_metrics.csv" 파일을 수집·합산한
    1행 DataFrame을 반환합니다.

    변경 요지:
    - root_dir 및 그 하위 전체(rglob)에서 *_metrics.csv를 찾도록 확장하여
      멀티-저널/멀티-연도에서 metrics 파일 누락 문제를 해결합니다.
    - 읽기 시 on_bad_lines='warn' 로 유연하게 처리.
    - 합산 후 DOI/ROR 보강률을 (보강된 수 / (보강된 수 + 원래 누락 수))로 계산합니다.
    - 반환 형식은 원래와 호환되도록 한 행 DataFrame을 반환합니다.
    """
    import pandas as pd
    import csv, re
    from pathlib import Path

    # 헬퍼: 숫자 변환 (None/빈문자열 -> None)
    def _to_num(s):
        try:
            if s is None or s == "":
                return None
    ...
    def _fmt_pct_ratio(numer, denom):
        try:
            if denom == 0:
                return "0.00%"
            return f"{(float(numer) / float(denom) * 100):.2f}%"
        except Exception:
            return "0.00%"

    # 1) 각 실행의 root_dir, prefix_all, y0, y1 메타 추출 (기존 방식 유지)
    roots_with_meta = []
    ...
    # 2) 모든 root_dir의 하위에서 *_metrics.csv 파일 수집 (중복 제거)
    metric_paths = []
    seen = set()
    for (root_dir, prefix_all, y0, y1) in roots_with_meta:
        if not root_dir.exists():
            continue
        for mf in root_dir.rglob("*_metrics.csv"):
            try:
                rp = mf.resolve()
            except Exception:
                rp = mf
            if rp in seen:
                continue
            seen.add(rp)
            metric_paths.append(mf)
    metric_paths = sorted(metric_paths)
    ...
    # 4) 각 metrics 파일 읽고 합산 (유연한 읽기)
    for mf in metric_paths:
        record = {}
        try:
            # 시도: pandas로 읽기 (일반적인 케이스)
            ...
            if "key" in df.columns and "value" in df.columns:
                ...
                for r in df.itertuples(index=False):
                    # 유연하게 첫 두 컬럼을 키/값으로 처리
                    if len(r) >= 2:
                        # r[0] may be attribute name; safer to use tuple access
            ...
            for k, v in rows:
                ...
                if num is not None:
                    acc[k] = acc.get(k, 0) + num
        except Exception as e:
            # 읽기 실패시 에러를 기록하되 전체 합산은 계속 진행
            record["_error"] = str(e)
        per_file.append((mf, record))

    # 5) 비율 재계산: DOI/ROR 보강률은 '보강된 수 / (보강된 수 + 누락 수)'로 계산
    ...
    acc["doi_enrich_rate"] = _fmt_pct_ratio(doi_enriched, doi_enriched + doi_missing)
    acc["ror_enrich_rate"] = _fmt_pct_ratio(ror_enriched, ror_enriched + ror_missing)

    # 6) editorial / authorships 관련 컬럼이름 일관화 유도 (가능하면 존재하는 키를 복사)
    # (기존 코드가 'authorships_removed_empty_list' 같은 키를 기대하므로 보장)
    if "authorships_removed_empty_list" not in acc:
        # 만약 다른 이름으로 들어와 있으면 그 값을 사용하도록 시도 (예: 'authorships_removed')
        if "authorships_removed" in acc:
            acc["authorships_removed_empty_list"] = acc["authorships_removed"]
        else:
            acc["authorships_removed_empty_list"] = acc.get("authorships_removed_empty_list", 0)

    # 7) 최종 1행 DataFrame 생성 — 컬럼 순서는 단일 Summary와 최대한 일치
    preferred_order = [
        "total_collected", "final_csv_rows", "authorships_removed_empty_list",
        "doi_missing", "doi_enriched", "doi_enrich_rate",
        "ror_missing", "ror_enriched", "ror_enrich_rate",
    ...
    # ensure every preferred key exists in acc
    for k in preferred_order:
        if k not in acc:
            acc[k] = 0 if not k.endswith("_rate") else "0.00%"

    # 정리: 숫자는 그대로, 비율은 문자열로 유지(원래의 single-summary와 호환)
    final_row = {k: acc.get(k, 0) for k in preferred_order}
