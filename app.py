...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    ...
    - recurses each final CSV's parent and its 'workdir_tmp' for all '*_metrics.csv'
    - accepts multiple metric key name variants and aggregates numeric keys
    - recomputes rates (DOI/ROR) from aggregated counts (do not trust per-file rates)
    - returns a one-row DataFrame with the UI's expected columns
    """
    import pandas as pd
    from pathlib import Path
    import csv, sys, ast, json, re
    ...
    # 1) collect directories to scan (parents + workdir_tmp)
    scan_dirs = set()
    ...
    # 2) find all *_metrics.csv under those dirs
    metrics_files = []
    for d in scan_dirs:
        if not d.exists():
            continue
    ...
    # Debug: list found files so you can verify
    print(f"[summary_multi] found {len(metrics_files)} metrics files:")
    for mf in metrics_files:
        print("  -", mf)

    # 3) aggregate numeric values from metrics files, tolerant to column-name variants
    agg = {}
    for mf in metrics_files:
        try:
            ...
            try:
                ...
                for k, v in rows:
                    ...
                    if num is not None:
                        ...
                        elif kn in ("authorships_removed_empty_list","editorial_removed"):
        ...
        # get rows from dataframe - support 'key'/'value' layout or simple two-col
        ...
        for k, v in rows:
            ...
            num = to_float_safe(v)
            if num is None:
                # skip non-numeric aggregated values (we'll re-calc rates)
                continue
            # canonicalize key names: map a variety of names to canonical agg keys
    ...
    # 4) compute ror_missing_before by scanning final CSVs authorships (best-effort)
    ...
    # 5) compute final aggregated fields and derived rates (always recompute rates)
    ...
    # DOI rate: enriched / (missing + enriched)
    doi_rate = "0.00%"
    denom = (doi_missing or 0) + (doi_enriched or 0)
    if denom:
        doi_rate = f"{(100.0 * (doi_enriched or 0) / denom):.2f}%"

    # ROR rate: use ror_missing from metrics if available; if not, use ror_missing_before_total
    ror_rate = "0.00%"
    denom_r = (ror_missing or 0) + (ror_enriched or 0)
    # try to repair ror_enriched from before/after if zero
    ...
    # try to also compute node/edge counts (best-effort) using existing helpers
    try:
        nodes = set(); edges = set()
        for p in sel_csv_list:
            try:
