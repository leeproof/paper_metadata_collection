...
def diag_metrics_ui():
    ...
    st.subheader("진단 도구: metrics 파일/aggregated 파일, FORCE_JOURNAL_KEY 점검")

    base = Path(".")
    # 1) 파일 수 및 폴더별 개수
    metrics_files = sorted(base.rglob("*_metrics.csv"))
    agg_files = sorted(base.rglob("*metrics_aggregated*.csv"))
    st.write(f"전체 *_metrics.csv 파일 수: **{len(metrics_files)}**")
    st.write(f"전체 metrics_aggregated 파일 수: **{len(agg_files)}**")
    ...
    # 3) aggregated 파일 목록과 샘플 내용
    if agg_files:
        st.write("---")
        st.write("발견된 metrics_aggregated 파일 (최대 10개):")
        for f in agg_files[:10]:
            st.write(f"- {f}")
        st.write("샘플 aggregated(첫 파일) 내용:")
        try:
            t = agg_files[0].read_text(encoding="utf-8", errors="replace")
            st.code("\n".join(t.splitlines()[:80]), language="text")
        except Exception as e:
            st.write(" (읽기 실패) ", e)
    else:
        st.write("metrics_aggregated 파일 없음")
...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    ...
    editorial_removed = int(acc.get("authorships_removed_empty_list", acc.get("authorships_removed", 0) or 0))
...
def app():
    ...
    with st.sidebar.expander("Diagnostics (Dev)", expanded=False):
        ...
        # 아래 체크박스를 켜면 브라우저에서 진단 도구가 실행됩니다 (터미널 불가 시 유용)
    if st.sidebar.checkbox("Run diagnostics (list metrics files, check FORCE_JOURNAL_KEY)"):
        diag_metrics_ui()
