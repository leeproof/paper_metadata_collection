...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    ...
    try:
        ...
        if have_cent_nodes:
            node_set = cent_nodes
            edge_count = len({(a,b) for (a,b) in set(all_pairs) if a in node_set and b in node_set})
        else:
...
def _summary_metrics_common(final_csv_path: str):
    ...
    if prefix_all and (y0 is not None) and (y1 is not None):
        allowed_names = {f"{prefix_all}_{yy}_{yy}_metrcis.csv" for yy in range(int(y0), int(y1) + 1)}
...
def app():
    ...
    if run_btn:
        ...
        if view == "Summary":
            _render_summary_unified(sel["csv"])
