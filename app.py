...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    """
    여러 실행(여러 저널 × 여러 연도)의 최종 CSV 목록을 받아,
    각 실행의 storage 하위(및 workdir_tmp)에서 해당 실행에 대응하는
    per-year metrics.csv 파일들을 찾아 정확히 합산한 1행 DataFrame을 반환합니다.

    변경 핵심:
    - allowed_names 는 '개별 저널 키' 기반으로 생성(합쳐진 prefix_all 사용하지 않음).
    - 검색 디렉터리는 각 final_csv의 부모 디렉터리와 그 하위 workdir_tmp 를 모두 검사.
    - 파일명 완전일치 필터 사용(그대로)하되, 허용 집합(allowed_names)에 모든 저널/연도 파일명을 포함.
    """
    import pandas as pd
    import ast, json, re
    from pathlib import Path
    ...
    # --- 1) 입력 final CSV 목록 파싱 및 (root dirs, 연도, 개별 키) 수집
    csv_paths = [Path(p) for p in sel_csv_list]
    roots_meta = []  # list of (final_csv_path, root_dir, keys_list, y0, y1)
    for final_csv in csv_paths:
        try:
            # new 모듈의 파서(파일명에서 key 목록과 연도 범위 추출) 재사용
            import new as _new_mod
            keys, y0, y1 = _new_mod._parse_keys_years_from_final(str(final_csv))
        except Exception:
            # fallback: 기존 방식으로 prefix_all/years 추출
            base_path = final_csv.with_suffix("")
            root_base = re.sub(r'_(?:ror(?:_extract(?:_name)?)?)(?:_summary)?$', '', str(base_path))
            m = re.search(r"_(\d{4})_(\d{4})$", root_base)
            if not m:
                # 못 파싱하면 건너뜀
                continue
            y0, y1 = int(m.group(1)), int(m.group(2))
            # prefix_all은 문자열 끝에서 연도부분 제거하면 남음 -> split('-')로 keys 추정
            prefix_all = Path(root_base).name[:-(len(m.group(0)))]
            keys = prefix_all.split("-") if prefix_all else [prefix_all]

        roots_meta.append((final_csv, final_csv.parent, keys, int(y0), int(y1)))
    ...
    # --- 2) allowed_names 집합 생성: 모든 키 × 모든 연도 조합의 metrics 파일명 포함
    allowed_names = set()
    for (_final_csv, _root_dir, keys, y0, y1) in roots_meta:
        for key in keys:
            for yy in range(int(y0), int(y1) + 1):
                allowed_names.add(f"{key}_{yy}_{yy}_metrics.csv")

    # --- 3) metrics 파일 위치 검색(각 root_dir 및 workdir_tmp 하위에서 rglob)
    search_dirs = set()
    for (_final_csv, root_dir, _keys, _y0, _y1) in roots_meta:
        search_dirs.add(Path(root_dir))
        search_dirs.add(Path(root_dir) / "workdir_tmp")

    acc = {
        "total_collected": 0,
        "final_csv_rows": 0,
        "authorships_removed_empty_list": 0,
        "doi_missing": 0,
        "doi_enriched": 0,
        "ror_missing": 0,
        "ror_enriched": 0,
        "ror_missing_after_extract": 0,
        "ror_missing_before_extract": 0,
    }
    seen = set()
    for base in search_dirs:
        ...
        for mf in base.rglob("*_metrics.csv"):
            ...
            try:
                ...
                # 읽기 실패는 건너뜀(로그를 남겨도 좋음)
                continue

    # --- 4) real4 전/후 ROR 결측(aggregate) — final CSV들에서 직접 계산 (원래 로직 유지)
    ...
    try:
        ...
        for final_csv in csv_paths:
            try:
                tmp = pd.read_csv(final_csv, dtype={"authorships": str})
    ...
    # --- 5) 노드/엣지 계산 (org_names 유니온 + 중앙성 CSV 우선)
    ...
    try:
        ...
        for final_csv in csv_paths:
            ...
            # 중앙성 CSV 우선 (기존 헬퍼 사용)
            try:
                cent_csv = _find_centrality_csvs(str(final_csv))
    ...
    # --- 6) 비율 재계산 (DOI / ROR) — 기존 방식(합산 기준)
    doi_missing_sum = acc.get("doi_missing", 0)
    doi_enriched_sum = acc.get("doi_enriched", 0)
    ror_missing_after = acc.get("ror_missing_after_extract", 0)
    ror_enriched_sum = acc.get("ror_enriched", 0)

    # DOI rate: enriched / (missing + enriched)
    doi_rate = 0.0
    denom = (doi_missing_sum or 0) + (doi_enriched_sum or 0)
    if denom:
        doi_rate = 100.0 * (doi_enriched_sum or 0) / denom

    # ROR enriched calc fallback
    ror_enriched_calc = ror_enriched_sum
    if (not ror_enriched_calc) and (ror_missing_before_total is not None) and (ror_missing_after is not None):
        ror_enriched_calc = max(0, (ror_missing_before_total or 0) - (ror_missing_after or 0))

    ror_rate = 0.0
    denom_r = (acc.get("ror_missing", 0) or 0) + (ror_enriched_calc or 0)
    if denom_r:
        ror_rate = 100.0 * (ror_enriched_calc or 0) / denom_r

    # --- 7) 최종 출력 포맷 (단일 Summary와 컬럼 순서/포맷 일치)
    data = {
        "전체 수집 논문 수": acc.get("total_collected", 0),
        "Editorial Material 삭제 수": acc.get("authorships_removed_empty_list", 0),
        "DOI 결측 수": doi_missing_sum,
        "DOI 보강 수(합산)": (doi_enriched_sum or 0),
        "DOI 보강률": _fmt_pct(doi_rate),
        "ROR ID 결측 수": ror_missing_before_total,
        "ROR ID 보강 수": ror_enriched_calc or 0,
        "ROR ID 보강률": _fmt_pct(ror_rate),
        "노드 수": node_count,
        "엣지 수": edge_count,
    }
