...
def _summary_metrics_common_multi(sel_csv_list: list[str]):
    ...
    - 각 최종 CSV에 대해 new.py의 build_summary_df_from_metrics_for_final()을 호출
    - 숫자 항목만 합산, 비율은 합산 결과로 재계산
    - 파일 저장/중간산출 없음 (메모리 직행)
    """
    import pandas as pd
    from new import build_summary_df_from_metrics_for_final
    ...
    for path in sel_csv_list:
        try:
            df = build_summary_df_from_metrics_for_final(path)
...
def _summary_metrics_common(final_csv_path: str):
    ...
    acc, found = sum_metrics([
        ...
        "authorships_removed_empty_list", "autrhoshipss_removed",
...
def app():
    ...
    def _render_summary_block(sel_csv_or_list):
        ...
        with st.expander("Summary", expanded=True):
            ...
            # ---------------------- Top10 노드/엣지 복원 ----------------------
            st.markdown("### Top 10 노드 / 엣지")

        def _single_top10(sel_csv_path: str):
            """
            기존 단일용 Top10 생성 헬퍼가 있으면 그걸 호출하고,
            없다면 안전한 최소 버전으로 기관/엣지 빈도 Top10을 만든다.
            """
            try:
                # 기존 단일용 헬퍼가 있으면 그대로 사용
                if '_top10_from_final_csv' in globals():
                    nodes_df, edges_df = _top10_from_final_csv(sel_csv_path)
                    return nodes_df, edges_df
            except Exception:
                pass

            # 최소 안전 버전(컬럼명 변경 시 맞춰주세요)
            try:
                df = pd.read_csv(sel_csv_path, dtype=str, encoding="utf-8-sig")
            except Exception as e:
                return None, None

            # 노드: 기관명 빈도 Top10
            node_col = "institution_name" if "institution_name" in df.columns else None
            nodes_df = None
            if node_col:
                nodes_df = (
                    df[node_col].dropna().astype(str).str.strip()
                      .value_counts().head(10).rename_axis("node")
                      .reset_index(name="count")
                )

            # 엣지: 저자 소속 ROR 페어로 단순 동출현 집계 (최소 구현)
            # 필요한 컬럼이 없다면 None
            edges_df = None
            if {"source_ror", "target_ror"}.issubset(df.columns):
                grp = (
                    df.dropna(subset=["source_ror", "target_ror"])
                      .groupby(["source_ror", "target_ror"]).size()
        ...
        if isinstance(sel_csv_or_list, (list, tuple)):
            # 멀티: 각 파일의 Top10을 합쳐서 다시 Top10
            agg_nodes = {}
            agg_edges = {}

            for p in sel_csv_or_list:
                nd, ed = _single_top10(p)
                if nd is not None and not nd.empty:
                    for _, r in nd.iterrows():
                        agg_nodes[r["node"]] = agg_nodes.get(r["node"], 0) + int(r["count"])
                if ed is not None and not ed.empty:
                    for _, r in ed.iterrows():
                        key = (str(r["source_ror"]), str(r["target_ror"]))
                        agg_edges[key] = agg_edges.get(key, 0) + int(r["weight"])

            # dict → DF (Top10)
            if agg_nodes:
                nodes_df = (pd.DataFrame(
                    [{"node": k, "count": v} for k, v in agg_nodes.items()]
                ).sort_values("count", ascending=False).head(10))
                st.markdown("**Top 10 Nodes (merged)**")
                st.dataframe(nodes_df, use_container_width=True)
            else:
                st.info("노드 정보를 만들 수 없습니다(컬럼 누락 가능).")

            if agg_edges:
                edges_df = (pd.DataFrame(
                    [{"source_ror": k[0], "target_ror": k[1], "weight": v} for k, v in agg_edges.items()]
                ).sort_values("weight", ascending=False).head(10))
                st.markdown("**Top 10 Edges (merged)**")
                st.dataframe(edges_df, use_container_width=True)
            else:
                st.info("엣지 정보를 만들 수 없습니다(컬럼 누락 가능).")

        else:
            # 단일: 기존 단일용 그대로
            nd, ed = _single_top10(sel_csv_or_list)
            if nd is not None and not nd.empty:
