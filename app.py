...
def _load_mapped_affiliations_for_final(final_csv_path: str) -> list[str]:
    """
    final_csv_path(…_ror_extract_name.csv)와 같은 prefix를 가진
    '<prefix>_mapped_affiliations.csv' 파일에서
    normalized_affiliation 리스트를 읽어온다.
    (이름 규칙은 프로젝트에 맞게 조정)
    """
    ...
    candidates = [
        base.with_name(f"{stem}_mapped_affiliations.csv"),
        base.parent / "workdir_tmp" / f"{stem}_mapped_affiliations.csv",
    ]
