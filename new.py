...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path=None):
    ...
    totals = {
        ...
        "id_pattern_removed": 0.0,              # ★ 별도 집계 버킷 추가
    ...
    pd.DataFrame([{
        "최초 수집 논문 수": _ival(tot.get("json_rows", tot["total_collected"])),  # json_rows 우선
        "최종 수집 논문 수": _ival(tot["final_csv_rows"]),
        "Editorial Material 삭제 수": _ival(tot["authorships_removed"]),
        "ID 패턴 불일치 삭제 수": _ival(tot["id_pattern_removed"]),
