...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str | None = None):
    """
    저장 없이 메모리로만 per-year '*_metrics.csv'를 합산하여 summary 생성.
    - 멀티저널×멀티연도: 최종 CSV 파일명에서 (keys, years) 전 조합을 커버
    - Editorial(= authorships 빈 리스트 삭제 수): 'authorships_removed' 계열을 합산
    - DOI/ROR 보강률: enriched / (missing + enriched)
    - ROR 보정: before - after 만큼 enriched로 이동
    - out_csv_path가 주어지면 CSV로 저장도 수행
    - 항상 pandas.DataFrame을 반환 (후방 호환 + app.py 기대치 일치)
    """
    import pandas as pd
    from pathlib import Path
    ...
    totals = {
        "total_collected": 0.0,
        "final_csv_rows": 0.0,
        "authorships_removed": 0.0,  # Editorial
    ...
    for _, _, m in _iter_metrics_dfs_from_final(final_csv_path):
        ...
        for _, row in m.iterrows():
            ...
            elif k == "json_rows":                     # ★ 얇은 매핑 추가 (집계 정확도 향상)
                totals["total_collected"] += v
            elif k in totals:
                totals[k] += v

    # 아무 것도 못 읽었으면 빈 요약 DF 반환
    if not got_any:
        df = pd.DataFrame([{
            "전체 수집 논문 수": 0,
    ...
    # ROR 보정: before - after → enriched로 이동, missing에서 차감(하한 0)
    delta = max(0.0, totals["ror_missing_before_extract"] - totals["ror_missing_after_extract"])
    if delta > 0:
        totals["ror_enriched"] += delta
        totals["ror_missing"] = max(0.0, totals["ror_missing"] - delta)

    # 비율 재계산(0 분모 방지)
    doi_denom = totals["doi_missing"] + totals["doi_enriched"]
    ror_denom = totals["ror_missing"] + totals["ror_enriched"]
    doi_rate = f"{(100.0 * totals['doi_enriched'] / doi_denom):.2f}%" if doi_denom else "0.00%"
    ror_rate = f"{(100.0 * totals['ror_enriched'] / ror_denom):.2f}%" if ror_denom else "0.00%"

    # 최종 1행 DF
    df = pd.DataFrame([{
        "전체 수집 논문 수": int(totals["total_collected"]),
        "Editorial Material 삭제 수": int(totals["authorships_removed"]),
        "최종 CSV 행 수(합계)": int(totals["final_csv_rows"]),
        "DOI 결측 수(합산)": int(totals["doi_missing"]),
        "DOI 보강 수(합산)": int(totals["doi_enriched"]),
        "DOI 보강률": doi_rate,
        "ROR ID 결측 수(합산)": int(totals["ror_missing"]),
        "ROR ID 보강 수(합산)": int(totals["ror_enriched"]),
        "ROR ID 보강률": ror_rate,
    }])

    # 요청 시 CSV 저장(후방 호환: run_pipeline_cached에서 사용 중)
    if out_csv_path:
        df.to_csv(out_csv_path, index=False, encoding="utf-8-sig")

    # 항상 DF 반환 → app.py의 기대에 부합
    return df
...
def _gdrive_download_metric_by_key_exact_path(key: str, year: int, local_path: Path) -> bool:
    ...
    폴더는 '존재하면' 탐색하고, 없으면 실패 처리(빈 폴더 생성 X).
    """
    try:
        root_id = _get_gdrive_root_folder_id()
        if not root_id:
        ...
        key_folder_id = _find_folder(drive, root_id, key)
        if not key_folder_id:
            return False
        year_folder_id = _find_folder(drive, key_folder_id, str(year))
...
def _gdrive_read_metric_csv_by_key_exact_path_to_df(key: str, year: int):
    """ROOT/<key>/<year>/<key>_<year>_<year>_metrics.csv → DF (로컬 저장 없이 메모리로 읽기)"""
    import pandas as pd
    try:
        root_id = _get_gdrive_root_folder_id()
        if not root_id:
