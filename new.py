...
def make_html_from_csv(final_csv_path: str) -> str:
    ...
    하나의 HTML에 두 뷰를 나란히 담아 반환(함수 시그니처/리턴타입 변경 없음).
    """
    ...
    # 개별 시각화(임시) 파일 경로
    html_degree = f"{base}_degree_network_tmp.html"
    html_eigen  = f"{base}_eigenvector_network_tmp.html"

    # 1) Degree 뷰 생성(권장: 크기=degree, 색=eigenvector)
    real6.main(
    ...
    # 2) Eigenvector 뷰 생성(권장: 크기=eigenvector, 색=degree)
    real6.main(
        input_csv=final_csv_path,
    ...
    deg_html = _read_text(html_degree)
    eig_html = _read_text(html_eigen)

    # Pyvis 기본 컨테이너/변수명 충돌 회피 (아주 단순한 치환)
    # - 컨테이너 id: mynetwork -> mynetwork_deg / mynetwork_eig
    # - 변수명: nodes/edges/network -> *_deg / *_eig
    def _massage(src: str, suffix: str) -> str:
        rep = src

        # 1) 컨테이너 id
        rep = rep.replace('id="mynetwork"',  f'id="mynetwork_{suffix}"')
        rep = rep.replace("id='mynetwork'",  f"id='mynetwork_{suffix}'")

        # 2) getElementById
        rep = rep.replace("getElementById('mynetwork')",  f"getElementById('mynetwork_{suffix}')")
        rep = rep.replace('getElementById("mynetwork")',  f'getElementById("mynetwork_{suffix}")')

        # 3) 전역 변수 충돌 방지 (정확 패턴 위주)
        rep = rep.replace('var nodes =',    f'var nodes_{suffix} =')
        rep = rep.replace('var edges =',    f'var edges_{suffix} =')
        rep = rep.replace('var network =',  f'var network_{suffix} =')
        rep = rep.replace('var data =',     f'var data_{suffix} =')
        rep = rep.replace('var options =',  f'var options_{suffix} =')

        # 4) data 객체 조립부의 참조 치환
        rep = rep.replace('nodes,',         f'nodes_{suffix},')
        rep = rep.replace('edges,',         f'edges_{suffix},')
        rep = rep.replace('nodes:',         f'nodes: nodes_{suffix},')
        rep = rep.replace('edges:',         f'edges: edges_{suffix},')

        # 5) network 생성부 파라미터 치환
        rep = rep.replace('new vis.Network(container, data, options)',
                        f'new vis.Network(container, data_{suffix}, options_{suffix})')

        # 6) 남아 있을 수 있는 mynetwork 문자열(따옴표 이슈)
        rep = rep.replace('mynetwork"',     f'mynetwork_{suffix}"')
        rep = rep.replace("mynetwork'",     f"mynetwork_{suffix}'")

        return rep

    # head/body 분리보다는 한 페이지에 스크립트 2개를 그대로 넣되 id/변수만 바꿔 중복 회피
    deg_block = _massage(deg_html, "deg")
    eig_block = _massage(eig_html, "eig")

    # 아주 단순하게 공용 HTML 래퍼 생성
...
<style>
  ...
  .panel {{ flex:1 1 48%; min-width:380px; border:1px solid #333; padding:8px; border-radius:8px; background:#151515; }}
  h3 {{ margin:6px 0 8px; font-weight:600; }}
</style>
</head>
<body>
  <div class="row">
...
"""
    ...
    # 임시 파일은 정리(선택)
    try:
        Path(html_degree).unlink(missing_ok=True)
        Path(html_eigen).unlink(missing_ok=True)
    except Exception:
        pass
