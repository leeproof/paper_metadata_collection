...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path=None):
    ...
    editorial = {"authorships_removed","authorships_removed_empty_list", "id_pattern_removed"}

    got_any=False
    for _,_,m in _iter_metrics_dfs_from_final(final_csv_path):
        if "key" not in m.columns or "value" not in m.columns:
            cols = list(m.columns)
            if len(cols) == 1:  # index에 key, 단일 컬럼이 value
    ...
    if not got_any:
        df = pd.DataFrame([{
            "전체 수집 논문 수":0,"Editorial Material 삭제 수":0,
            "최종 CSV 행 수(합계)":0,
            "DOI 결측 수(합산)":0,"DOI 보강 수(합산)":0,"DOI 보강률":"0.00%",
            "ROR ID 결측 수(합산)":0,"ROR ID 보강 수(합산)":0,"ROR ID 보강률":"0.00%",
        }])
    ...
    tot = totals.copy()
    # ROR before/after 보정
    delta = max(0, _ival(tot["ror_missing_before_extract"]) - _ival(tot["ror_missing_after_extract"]))
    if delta>0:
        tot["ror_enriched"] += delta
        tot["ror_missing"] = max(0, _ival(tot["ror_missing"]) - delta)
    ...
    df = pd.DataFrame([{
        "전체 수집 논문 수": _ival(tot["total_collected"]),
        "Editorial Material 삭제 수": _ival(tot["authorships_removed"]),
        "최종 CSV 행 수(합계)": _ival(tot["final_csv_rows"]),
