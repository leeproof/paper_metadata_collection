...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path=None):
    """
    per-year '*_metrics.csv' 합산 → 1행 Summary DataFrame 반환.
    - 기존 로직/변수명 유지
    - out_csv_path 지정 시 CSV 저장(후방 호환)
    - 항상 DataFrame 반환
    """
    import pandas as pd
    from pathlib import Path
    ...
    for _, _, m in _iter_metrics_dfs_from_final(final_csv_path):
        ...
        for _, row in m.iterrows():
            ...
            elif k == "json_rows":  # real2: 병합 JSON 개수 → 전체 수집 수로 합산
                totals["total_collected"] += v
            elif k in totals:
                totals[k] += v

    # 데이터 없음 → 빈 DF (요청 시 CSV 저장)
    if not got_any:
        df = pd.DataFrame([{
            "전체 수집 논문 수": 0,
    ...
    # ROR before/after 보정(없으면 0)
    ror_missing_bef   = _ival(totals["ror_missing_before_extract"])
    ror_missing_aft   = _ival(totals["ror_missing_after_extract"])
    delta = max(0, ror_missing_bef - ror_missing_aft)
    if delta > 0:
        ror_enriched += delta
        ror_missing = max(0, ror_missing - delta)
