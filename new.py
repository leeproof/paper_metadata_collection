...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str):
    ...
    final_rows = int(totals["final_csv_rows"]) if totals["final_csv_rows"] else 0
    doi_enriched = int(totals["doi_enriched"])
    ror_enriched = int(totals["ror_enriched"])

    df = pd.DataFrame([{
        "전체 수집 논문 수(최초)": int(totals["total_collected"]),
        "Editorial Material 삭제 수": int(totals["authorships_removed_empty_list"]),
        "최종 CSV 행 수(합계)": final_rows,
        "DOI 결측 수(합산)": int(totals["doi_missing"]),
        "DOI 보강 수(합산)": doi_enriched,
        "DOI 보강률": (f"{(doi_enriched / final_rows * 100):.2f}%"
                     if final_rows else "0.00%"),
        "ROR ID 결측 수(합산)": int(totals["ror_missing"]),
        "ROR ID 보강 수(합산)": ror_enriched,
        "ROR ID 보강률": (f"{(ror_enriched / final_rows * 100):.2f}%"
                        if final_rows else "0.00%"),
    }])
