...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str):
    ...
    - csv.reader로 헤더 유무/형식에 무관하게 안전 파싱
    """
    ...
    # csv.reader로 헤더 유무와 상관없이 (key, value) 쌍을 안전 누적
    for p in metric_paths:
        ...
        # 1) 헤더가 'key,value' 형태인지 판별
        start_idx = 0
        if len(rows[0]) >= 2:
            h0, h1 = str(rows[0][0]).strip().lower(), str(rows[0][1]).strip().lower()
            if h0 == "key" and h1 == "value":
                start_idx = 1  # 헤더 스킵

        # 2) 이후 모든 라인을 (key, value)로 간주하고 합산
        for i in range(start_idx, len(rows)):
            row = rows[i]
            if not row:
                continue
            k = row[0].strip() if len(row) > 0 and row[0] is not None else ""
            v = _to_num(row[1]) if len(row) > 1 else None
