...
def _get_service_account_info() -> Optional[dict]:
    """Secrets/ENV에서 단일 키 GDRIVE_SA_JSON만 읽어 JSON 파싱."""
    import json as _json
    raw = None
    if st is not None:
        try:
            raw = st.secrets["GDRIVE_SA_JSON"]
        except Exception:
            raw = None
    if raw is None:
        raw = os.getenv("GDRIVE_SA_JSON")
    if raw is None:
        print("[GDRIVE] GDRIVE_SA_JSON not found in secrets/env")
        return None
    ...
    try:
        ...
        if "client_email" in sa and "private_key" in sa:
            print("[GDRIVE] Loaded service account (client_email=%s)" % sa.get("client_email"))
            return sa
        print("[GDRIVE] JSON present but missing required keys:", list(sa.keys()))
        return None
    except Exception as e:
        print("[GDRIVE] JSON decode failed:", repr(e))
        # 디버깅 편의용 앞부분만 출력
