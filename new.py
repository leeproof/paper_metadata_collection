...
def _get_service_account_info() -> Optional[dict]:
    """
    (옵션 A) 오직 [gcp_service_account] TOML 섹션만 사용.
    - st.secrets["gcp_service_account"] 가 dict 로 전달됨
    - dict 인 경우 private_key 에 '\n' 이 들어있으면 실제 개행으로 정규화
    - 그 외는 ENV 백업(GDRIVE_SA_JSON 문자열 JSON)만 허용
    """
    raw = None
    if st is not None:
        try:
            raw = st.secrets["gcp_service_account"]  # TOML 섹션 → dict
        except Exception:
            raw = None
    if raw is None:
        raw = os.getenv("GDRIVE_SA_JSON")  # 선택적 백업(문자열 JSON)
        # 문자열 JSON일 때만 json.loads()
        if isinstance(raw, str):
            s = raw.strip()
            try:
                raw = _json.loads(s)
            except Exception as e:
                print("[GDRIVE] Failed to parse GDRIVE_SA_JSON:", repr(e))
                return None

    if not isinstance(raw, dict):
        print("[GDRIVE] gcp_service_account not found (dict expected).")
        return None
