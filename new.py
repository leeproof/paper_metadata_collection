...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str):
    """
    같은 폴더의 per-year metrics.csv들을 읽어 합산 Summary CSV를 생성.
    - 분모/정의 = 최종 CSV 기준으로 통일
    - 라벨: 'Editorial Material 삭제 수' (authorships 빈 배열 삭제 수와 동일)
    """
    metric_paths = sorted(Path(final_csv_path).parent.glob("*_metrics.csv"))
    # 합산 키
    agg_keys = [
        "total_collected",
    ...
    for p in metric_paths:
        try:
            ...
            continue
        d = dict(zip(m.get("key", []), m.get("value", [])))
        for k in agg_keys:
            v = d.get(k, None)
            if v is not None:
                try:
...
def _collect_merge(issns: List[str], year_start: int, year_end: int) -> Path:
    ...
    dfs = [pd.read_csv(p, engine="python", on_bad_lines="skip", encoding="utf-8-sig")
...
def run_pipeline_cached(issns: List[str], year_start: int, year_end: int,
    ...
    # reset existing forced key so we always compute a fresh key for this run
    global FORCE_JOURNAL_KEY
    FORCE_JOURNAL_KEY = None

    # compute fresh key from ISSN (bypass _key_name_for's early-return)
    if USE_JOURNAL_NAME_SLUG:
        FORCE_JOURNAL_KEY = _resolve_journal_name_slug(issns[0])
    else:
        FORCE_JOURNAL_KEY = issns[0]
