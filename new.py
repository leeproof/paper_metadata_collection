...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str):
    ...
    - 파일 형식은 모두 동일: (key,value) 컬럼
    - editorial은 'authorships_removed'로 집계하되, 동등 키('authorships_removed_empty_list')도 같은 항목으로 더함
    ...
    # 합산할 키(최종적으로 사용할 canonical 키들)
    totals = {
        "total_collected": 0.0,
        "authorships_removed": 0.0,            # ← editorial (canonical)
        "final_csv_rows": 0.0,
    ...
    # editorial 동등 키를 canonical로 흡수 (형식은 같지만 키 이름이 다를 수 있으므로 안전 흡수)
    editorial_equivs = {"authorships_removed", "authorships_removed_empty_list"}

    for p in metric_paths:
        try:
            # 형식 동일: key,value 컬럼. NaN 전파 방지 위해 문자열로 읽어 직접 파싱.
