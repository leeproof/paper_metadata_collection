...
def _get_service_account_info() -> Optional[dict]:
    """
    Streamlit Secrets 또는 ENV에서 서비스계정 JSON을 안전하게 파싱.
        우선순위:
        1) st.secrets["gdrive_service_account"] (문자열 또는 dict)
        2) st.secrets["gcp_service_account"]    (TOML 테이블)
        3) os.getenv("GDRIVE_SA_JSON")          (문자열 JSON)
    """
    import json as _json
    ...
    if raw is None:
        print("[GDRIVE] gdrive_service_account not found in secrets/env")
        return None

    # 이미 dict인 경우(일부 환경에서 이렇게 들어옴)
    ...
    try:
        sa = _json.loads(s)
        if "client_email" in sa and "private_key" in sa:
            return sa
        print("[GDRIVE] Parsed JSON but missing keys:", list(sa.keys()))
        return None
    except Exception as e:
        print("[GDRIVE] Failed to parse service account JSON:", repr(e))
...
def _gdrive_client() -> GoogleDrive:
    ...
    # 일부 환경에서 pydrive2가 oauth2client 스타일 속성을 참조하는 것 방지
    if not hasattr(gauth.credentials, "access_token_expired"):
        try:
            object.__setattr__(gauth.credentials, "access_token_expired", gauth.credentials.expired)
        except Exception:
            pass
