아래 전체 파일로 **그대로 교체**해서 쓰세요.
(다른 부분은 손대지 않고, `build_summary_from_metrics_for_final`만 aggregate.metrics 로직과 **완전히 동일하게** 맞춰 수정했습니다. 또한 빈 경로 로그의 `base_dir` 미정의 버그, 숫자 파싱 시 `,` 포함 값 처리도 반영했습니다.)
...
def _collect_metrics_paths_from_final(final_csv_path: str):
    """
    LOCAL_WORKDIR/<key>/<year>/<key>_<year>_<year>_metrics.csv 경로들을 모아 반환.
    """
    ...
    for key in keys:
        for y in range(y1, y2 + 1):
            p = base_dir / key / str(y) / f"{key}_{y}_{y}_metrics.csv"
            if p.exists():
                paths.append(p)
    return paths
...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str):
    """
    최종 CSV 이름에서 파싱한 (key, year)의 per-year *_metrics.csv 만 정확히 합산해서 summary 생성.
    - 수집 대상: _collect_metrics_paths_from_final(final_csv_path)
    - 산식: enriched / (missing + enriched)
    - ROR 보정: fixed = max(0, ror_missing_before_extract - ror_missing_after_extract)
                ror_enriched += fixed, ror_missing -= fixed
    - 숫자 파싱 시 '1,234' 같은 값도 안전 처리
    """
    from pathlib import Path as _Path

    base_dir = _Path(final_csv_path).parent
    metric_paths = _collect_metrics_paths_from_final(final_csv_path)

    # metrics 파일이 하나도 없으면 안전한 빈 summary 생성
    if not metric_paths:
        print(f"[WARN] build_summary: metrics 파일을 찾지 못했습니다: {base_dir}")
        ...
        _Path(out_csv_path).parent.mkdir(parents=True, exist_ok=True)
        df_empty.to_csv(out_csv_path, index=False, encoding='utf-8-sig')
        return out_csv_path

    # ---- aggregate.metrics와 동일한 키 세트 ----
    agg_keys = [
        "total_collected",
        "authorships_removed_empty_list",
        "final_csv_rows",
        "doi_missing", "doi_enriched",
        "ror_missing", "ror_enriched",
        "ror_missing_before_extract", "ror_missing_after_extract",
    ]
    totals = {k: 0.0 for k in agg_keys}

    def _to_num(x):
        """문자 숫자 안전 변환: '1,234' -> 1234, 공백/None -> None, 그 외 실패 -> None"""
        if x is None:
            return None
        s = str(x).strip()
        if s == "":
    ...
    for p in metric_paths:
        try:
            ...
            if "key" not in m.columns or "value" not in m.columns:
                ...
                m = pd.DataFrame(rows)
                if "key" not in m.columns or "value" not in m.columns:
                    print(f"[WARN] metrics 포맷 불명으로 스킵: {p}")
                    continue
    ...
    # ---- 집계값 정리 ----
    total_collected   = int(totals.get("total_collected", 0) or 0)
    editorial_removed = int(totals.get("authorships_removed_empty_list", 0) or 0)
    final_rows        = int(totals.get("final_csv_rows", 0) or 0)

    doi_missing  = int(totals.get("doi_missing", 0) or 0)
    doi_enriched = int(totals.get("doi_enriched", 0) or 0)

    ror_missing  = int(totals.get("ror_missing", 0) or 0)
    ror_enriched = int(totals.get("ror_enriched", 0) or 0)

    # ---- ROR before/after 보정 (aggregate 동일) ----
    ror_before = int(totals.get("ror_missing_before_extract", 0) or 0)
    ror_after  = int(totals.get("ror_missing_after_extract", 0) or 0)
    if ror_before or ror_after:
        fixed = max(0, ror_before - ror_after)
        ror_enriched += fixed
        ror_missing = max(0, ror_missing - fixed)

    # ---- rate 재계산: enriched / (missing + enriched) ----
