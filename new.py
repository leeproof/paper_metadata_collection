...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str):
    """
    per-year "*_metrics.csv"만 모아 합산하여 summary 생성.
    - 대상 수집: _collect_metrics_paths_from_final(final_csv_path)
    - dtype=str로 읽고, key/value는 strip() 후 안전 파싱
    - editorial: 동의어 키를 canonical("authorships_removed")로 합산
    ...
    agg_keys = [
        "total_collected",
        "authorships_removed",                 # ← canonical (editorial)
        "final_csv_rows",
    ...
    # editorial 동의어 → canonical 매핑
    editorial_alias = {
        "authorships_removed": "authorships_removed",
        "authorships_removed_empty_list": "authorships_removed",
        "authorships_removed_empty": "authorships_removed",
        # 혹시 있을 수 있는 변형들(공백/대소문자/하이픈 등)도 안전 처리
        " editorial_removed": "authorships_removed",
        "editorial_removed": "authorships_removed",
    }
    ...
    for p in metric_paths:
        ...
        # key/value를 문자열로 받아 **앞뒤 공백 제거** 후 처리
        for _, row in m.iterrows():
            raw_k = (row.get("key") or "").strip()
            v = _to_num(row.get("value"))
            if not raw_k or v is None:
                continue

            # editorial 동의어를 canonical로 통일
            k = editorial_alias.get(raw_k, raw_k)
    ...
    total_collected   = _ival(totals.get("total_collected"))
    editorial_removed = _ival(totals.get("authorships_removed"))
    final_rows        = _ival(totals.get("final_csv_rows"))

    doi_missing  = _ival(totals.get("doi_missing"))
    doi_enriched = _ival(totals.get("doi_enriched"))

    ror_missing  = _ival(totals.get("ror_missing"))
    ror_enriched = _ival(totals.get("ror_enriched"))

    # ROR before/after 보정
    ror_before = _ival(totals.get("ror_missing_before_extract"))
    ror_after  = _ival(totals.get("ror_missing_after_extract"))
