...
def build_summary_from_metrics_for_final(final_csv_path: str, out_csv_path: str):
    """
    final_csv_path 의 부모(LOCAL_WORKDIR) 및 하위 폴더에서 모든 "*_metrics.csv" 파일을 찾아 합산하여
    summary CSV를 생성합니다. (최소 변경 · 안전)
    - 하위폴더까지 검색: Path(...).rglob("*_metrics.csv")
    - 각 metrics 파일은 인코딩 'utf-8-sig' 로 읽고, 문제 라인은 경고 처리(on_bad_lines='warn')
    - metrics 파일이 전혀 없으면 0으로 채운 안전한 summary CSV 생성
    """
    import pandas as pd
    from pathlib import Path

    base_dir = Path(final_csv_path).parent
    metric_paths = sorted(base_dir.rglob("*_metrics.csv"))

    # metrics 파일이 하나도 없으면, downstream이 예외를 던지지 않도록 '0' 행 하나를 만들고 반환
    if not metric_paths:
        print(f"[WARN] build_summary: metrics 파일을 찾지 못했습니다: {base_dir} (하위폴더 포함 검색)")
        ...
        df_empty.to_csv(out_csv_path, index=False, encoding='utf-8')
        return out_csv_path

    # 합산할 키(원본 metrics 파일의 key/value 쌍에서 추출)
    agg_keys = [
        "total_collected",
    ...
    for p in metric_paths:
        try:
            # 파일을 관대하게 읽기(인코딩 명시, 문제 라인은 경고)
            m = pd.read_csv(p, encoding='utf-8-sig', on_bad_lines='warn')
        ...
        # metrics 파일은 key,value 형태를 기대: 이를 dict로 변환
        keys = list(m.get("key", []))
        vals = list(m.get("value", []))
    ...
    # 🔁 app.py 로직과 동일한 ROR 보정: before - after 만큼 enriched로 이동
    ...
    # 📐 분모 = missing + enriched (aggregate와 동일)
