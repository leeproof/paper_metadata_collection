...
def _get_service_account_info() -> Optional[dict]:
    """
    Streamlit Secrets 또는 ENV에서 서비스계정 정보를 안전하게 얻는다.
    우선순위(옵션 A):
      1) st.secrets["gcp_service_account"]  ← TOML 테이블(= 파이썬 dict)
      2) os.getenv("GDRIVE_SA_JSON")        ← (선택) 문자열 JSON fallback
    """
    import json as _json

    # 1) gcp_service_account (권장·우선)
    if st is not None:
        try:
    ...
    if raw is None:
        print("[GDRIVE] gcp_service_account not found in secrets; no ENV fallback present")
        return None
